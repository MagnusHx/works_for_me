...
def test_save_learning_curve_creates_outputs(tmp_path, monkeypatch):
	...
	monkeypatch.setattr(train_mod, "Path", lambda p=".": __import__("pathlib").Path(tmp_path) / p)
...
def test_train_one_epoch_and_evaluate_smoke():
	...
	loader = torch.utils.data.DataLoader(torch.utils.data.TensorDataset(data_x, data_y), batch_size=2)
	criterion = torch.nn.CrossEntropyLoss()
	optimizer = torch.optim.SGD(model.parameters(), lr=0.1)

	train_loss, train_acc = train_mod.train_one_epoch(
		model, loader, criterion, optimizer, device, log_every=0, scaler=None, autocast_kwargs={"enabled": False}
	)
	assert train_loss >= 0
