...
def evaluate_model(
    ...
    if num_classes > 0:
        for class_idx in range(num_classes):
            ...
            per_class[name] = (
                float(class_correct[class_idx].item() / denom) if denom > 0 else 0.0
            )
...
def build_loader(dataset, cfg: DictConfig) -> DataLoader:
    ...
    if num_workers > 0:
        loader_kwargs["prefetch_factor"] = int(
            getattr(cfg.dataloader, "prefetch_factor", 2)
        )
...
def main(
    ...
    typer.echo(
        f"Dataset size: {n_total} | Split ratios: train={cfg.splits.train}, val={cfg.splits.val}, seed={cfg.experiment.seed}"
    )
    typer.echo(f"Split sizes -> train: {n_train}, val: {n_val}, test: {n_test}")

    if n_test <= 0:
        raise RuntimeError(
            "Test split size must be positive. Adjust split ratios in the config."
        )
